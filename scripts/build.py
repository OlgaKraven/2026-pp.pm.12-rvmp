...
def load_course():
    ...
    # Publish every variant as an independent card. New courses store all 30 cards
    # directly; grouped legacy sources are expanded for compatibility.
    if all(re.match(r'Вариант\s+\d+', area.get('title', '')) for area in data['areas']):
        for area in data['areas']:
            number=re.search(r'Вариант\s+(\d+)',area['title']).group(1)
            area['anchor']=f'variant-{number}'
    else:
        variants=[]
        for area in data['areas']:
            for source_block in area['blocks']:
                number=re.search(r'Вариант\s+(\d+)',source_block['title']).group(1)
                parts=[]
                for line in source_block['text'].splitlines():
...
def pdf(path, data, pages, name):
    ...
    def frame(title, kicker, continuation=False):
        ...
        c.drawString(margin, 22, "ПП.ПМ.12 · программирование мобильных игр")
...
def build(data):
    ...
    archive=OUT/'downloads'/'PP12-CrystalRoute-UnityProject.zip'
    ...
    with zipfile.ZipFile(archive,'w',zipfile.ZIP_DEFLATED) as package:
        for source in sorted(project.rglob('*')):
            ...
            if source.is_dir() or source.name.startswith('.') or source.suffix in {'.log', '.csproj', '.sln'} or any(part in excluded for part in relative.parts):
                continue
            package.write(source,Path('CrystalRoutePP12')/relative)
