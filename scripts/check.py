...
expected={'assignment.pdf','guide.pdf','PP12-CrystalRoute-UnityProject.zip'}|code_names
...
assert (site/'downloads/PP12-CrystalRoute-UnityProject.zip').stat().st_size>1000
...
# Every runtime file must be taught, and generated archives must stay clean.
import zipfile
runtime_names={p.name for p in (ROOT/'unity-project/Assets/Scripts').glob('*.cs')}
assert runtime_names == code_names, 'Runtime code omitted from teaching material'
with zipfile.ZipFile(site/'downloads/PP12-CrystalRoute-UnityProject.zip') as archive:
    names=archive.namelist()
    forbidden={'Library','Temp','Logs','obj','UserSettings','Build','_Recovery'}
    assert not any(forbidden.intersection(n.split('/')) for n in names)
    assert any(n.endswith('Assets/Scenes/Main.unity') for n in names)
    assert any(n.endswith('Packages/manifest.json') for n in names)
    assert any(n.endswith('ProjectSettings/ProjectVersion.txt') for n in names)
assert all('ПП.ПМ.12' in (site/n).read_text() for n in ('index.html','lessons.html','areas.html'))
assert 'УП.ПМ.13 · оформление' not in (site/'index.html').read_text()
print('PASS: all runtime code covered, clean Unity archive, PP.PM.12 identity')
